...
echo "$($PSQL "SELECT t1.name AS name FROM games JOIN teams t1 ON games.winner_id = t1.team_id WHERE round='Eighth-Final' AND year=2014 UNION SELECT t2.name AS name FROM games JOIN teams t2 ON games.opponent_id = t2.team_id WHERE round='Eighth-Final' AND year=2014 ORDER BY name")"

echo -e "\nList of unique winning team names in the whole data set:"
echo "$($PSQL "SELECT DISTINCT(t1.name) AS name FROM games JOIN teams t1 ON games.winner_id = t1.team_id JOIN teams t2 ON games.opponent_id = t2.team_id ORDER BY name")"
